...
do  i=2,10*365
  ...
  flow_epi_x = flow_in_epi_x*flow_Tin(i)
  flow_epi_x = (flow_epi_x - flow_out_epi_x*temp_epil(i-1))/volume_e_x
  ...
  flow_hyp_x  = (flow_in_hyp_x*flow_Tin(i)+flow_out_hyp_x*temp_hypo(i-1))/volume_h_x
  temp_change_hyp(i) = flow_hyp_x  +  (dif_hyp_x/volume_h_x)  ! diffusion 
